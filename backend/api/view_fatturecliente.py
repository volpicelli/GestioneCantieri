...
from .scadenzariofatturecliente_serializer import ScadenzarioFattureClienteSerializer
from .fatturecliente_serializer import FattureClienteserializer
...
from home.models import ScadenzarioFattureCliente,FattureCliente,Azienda,CondizioniPagamento,Cliente
from home.models import FattureCliente,Azienda,CondizioniPagamento,Cliente

import json
from django.conf import settings


class ClienteFatturaCreate(APIView):
    fatture_serialize = FattureClienteserializer
    scadenza_serializer = ScadenzarioFattureClienteSerializer
    ...
    def post(self,request):
        ...
        if 'azienda' not in data.keys() :
            ...
            return Response(msg)
        if 'cliente' not in data.keys() :
            msg = 'Cliente non specificato'
            return Response(msg)
        ...
        if 'codpag' in data.keys():
            ...
            condizionipag = cliente.codpag
            tipologiapagamento = cliente.codpag.tipopag
            numerorate = cliente.codpag.numrate

        


        fattura = FattureCliente.objects.create(
            pagato=pagato,
            cliente=cliente,
            importo=importo,
        ...
        if 'scadenze' in data.keys():
            ...
            for scadenza in scadenze:
                ...
                sf = ScadenzarioFattureCliente.objects.create(
        ...
        c = self.cliente_serializer(cliente)
        resp['cliente'] = c.data
        scadenze =[]
        for one in fattura.fatturecliente_scadenzario.all():
            scadenze.append(self.scadenza_serializer(one).data)
        resp['scadenze'] = scadenze
        return Response(resp)


class ClienteFatturaUpdate(APIView):
    fatture_serialize = FattureClienteserializer
    scadenza_serializer = ScadenzarioFattureClienteSerializer
    ...
    def get(self,request,pk):
        try:
            fc = FattureCliente.objects.get(pk=pk)
        except ObjectDoesNotExist:
            return Response( f" Fattura {pk} non esiste ")        
        
        cliente = Cliente.objects.get(pk=fc.cliente.id)
        codpag = self.codpag_serializer(fc.codpag)


        sf = fc.fatturecliente_scadenzario.all()
        fatture_serializer = self.fatture_serialize(fc)
        #serializer.data['articoli'] =[]
        scadenze=[]
        #ars = Articoliserializer(ar,many=True)
        for one in sf:
            sfs = ScadenzarioFattureClienteSerializer(one)
    ...
    def post(self,request,pk):
        ...
        try:
            fattura = FattureCliente.objects.get(pk=pk)
        except ObjectDoesNotExist:
        ...
        sf = fattura.fatturecliente_scadenzario.all()
        lensf_indb = len(sf)
        ...
        os = self.fatture_serialize(fattura)
        resp['fattura']=os.data
        c = self.cliente_serializer(fattura.cliente)
        resp['cliente'] = c.data
        scadenze =[]
...
class ClienteFattureList(generics.ListCreateAPIView):
    queryset = FattureCliente.objects.all()
    serializer_class = FattureClienteserializer
    #permission_classes = [IsAuthenticated]

    

class ClienteFattureDetail(generics.RetrieveUpdateDestroyAPIView):
    queryset = FattureCliente.objects.all()
    serializer_class = FattureClienteserializer
    scadenza_serializer = ScadenzarioFattureClienteSerializer
...
class ScadenzarioFattureClienteList(generics.ListCreateAPIView):
    queryset = ScadenzarioFattureCliente.objects.all()
    serializer_class = ScadenzarioFattureClienteSerializer
...
class ScadenzarioFattureClienteDetail(generics.RetrieveUpdateDestroyAPIView):
    queryset = ScadenzarioFattureCliente.objects.all()
    serializer_class = ScadenzarioFattureClienteSerializer
    
    def destroy(self, request, pk,*args, **kwargs):
        #pk = self.kwargs.get('pk')
        object = ScadenzarioFattureCliente.objects.get(pk=pk).delete() #kwargs['pk'])
        #serializer = self.serializer_class(object)
        return Response({'Msg':'OK '+str(pk) +' deleted'})

    def retrieve(self, request, pk,*args, **kwargs):
        #pk = self.kwargs.get('pk')
        object = ScadenzarioFattureCliente.objects.get(pk=pk) #kwargs['pk'])
